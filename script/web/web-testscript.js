  ...
  describe("Creating a reward", () => {
    it("Login to dashboard then to create new rewards", async () => {
      ...
      await driver
        ...
        .sendKeys(page_variables.usernameReward);
      await driver
        .findElement(By.id("password"))
        .sendKeys(page_variables.passwordReward);
      await driver.findElement(By.xpath(web_elements.loginButton)).click();
      // Login forward
      await driver.manage().setTimeouts({ implicit: 5000 });

      await driver
        .findElement(By.xpath(web_elements.createNewRewardButtonElement))
        .click();
      ...
      await driver
        .findElement(By.name(web_elements.rewardName))
        .sendKeys(rewardName);

      await driver
        .findElement(By.xpath(web_elements.nextButtonElement + "[2]"))
        .click(); // Next button
      await driver.manage().setTimeouts({ implicit: 5000 });
      ...
      await driver
        .findElement(By.xpath(web_elements.datePickerElement))
        .click();

      await driver.findElement(By.xpath(web_elements.dayElement)).click(); // 17 Jan 2024
      await driver
        .findElement(By.xpath(web_elements.nextButtonElement))
        .click(); // Next button

      await driver.manage().setTimeouts({ implicit: 5000 });

      await driver
        .findElement(By.xpath(web_elements.saveButtonElement))
        .click(); // Save button
      ...
      expect(currentUrl).toEqual(page_variables.createNewRewardUrl);
      await driver
        .findElement(By.xpath(web_elements.privateTypebuttonElement))
        .click(); // select Private type
      const tagDeltaY = await driver
        .findElement(By.xpath(web_elements.tagLabelElement))
        .getRect();
...
export function upload_bulk_list() {
  describe("Upload a file in bulk list", () => {
    ...
    it("login and upload bulk list", async () => {
      ...
      await driver
        .findElement(By.id("email"))
        .sendKeys(page_variables.usernameAdmin);
      await driver
        .findElement(By.id("password"))
        .sendKeys(page_variables.passwordAdmin);
      await driver.findElement(By.xpath(web_elements.loginButton)).click();
      // Login forward
      await delay(5000);
      await driver
        .findElement(By.xpath(web_elements.bulkAtionsTabElement))
        .click();
      await delay(2000);
      ...
      await driver
        .findElement(By.xpath(web_elements.uploadButtonElement))
        .click();
      await delay(2000);

      await driver
        .findElement(By.xpath(web_elements.uploadFile))
        .sendKeys(file_variables.imageFile);
      await driver
        .findElement(By.xpath(web_elements.uploadFileButtonElement))
        .click(); // upload file
      await delay(1500);

      let errorMessage = await driver
        .findElement(By.className(web_elements.popupMessage))
        .isDisplayed();
      expect(errorMessage).toBe(true);
      await delay(2000); // expect cannot upload other file than .txt, .xlsx, .csv
      await driver.findElement(By.xpath(web_elements.closeUploadDialogButtonElement)).click();

      async function uploadFileFunction(fileType) {
        await delay(2000);
        await driver
          .findElement(By.xpath(web_elements.uploadButtonElement))
          .click();
        await delay(2000);
        let isRemoveFileButtonPresent;
        try{
            await driver.findElement(By.xpath(web_elements.removeFileButton)).isDisplayed();
            isRemoveFileButtonPresent = true;
        }catch(err){
            isRemoveFileButtonPresent = false;
        }

        if(isRemoveFileButtonPresent) await driver.findElement(By.xpath(web_elements.removeFileButton)).click();
        await delay(2000);
        await driver
          .findElement(By.xpath(web_elements.uploadFile))
          .sendKeys(fileType); // upload .xlsx file
        await delay(2000);
        await driver
          .findElement(By.xpath(web_elements.uploadFileButtonElement))
          .click();
        ...
        let createdRecord = (
          await driver
            .findElement(By.xpath(web_elements.createdRecordNameUploadFile))
            .getText()
        ).toString();
        expect(createdRecord).toEqual(getFileName(fileType));
      ...
      // -- upload csvFile
      await uploadFileFunction(file_variables.csvFile);
      // -- upload xlsxFile
      await uploadFileFunction(file_variables.xlsxFile);
      // -- upload txtFile
      await uploadFileFunction(file_variables.txtFile);
